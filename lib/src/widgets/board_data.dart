...
class FlowBoardController extends ChangeNotifier
    with EquatableMixin, BoardPhantomControllerDelegate, ReoderFlexDataSource {
  final List<FlowBoardGroupData> _groupDatas = [];
  ...
  /// Returns the unmodifiable list of [FlowBoardGroupData]
  UnmodifiableListView<FlowBoardGroupData> get groupDatas =>
  ...
  final LinkedHashMap<String, FlowBoardGroupController> _groupControllers =
      LinkedHashMap();
  ...
  void addGroup(FlowBoardGroupData groupData, {bool notify = true}) {
    if (_groupControllers[groupData.id] != null) return;

    final controller = FlowBoardGroupController(groupData: groupData);
  ...
  /// [notify] should set to false. Default value is true.
  void addGroups(List<FlowBoardGroupData> groups, {bool notify = true}) {
  ...
  /// groups or get ready to reinitialize the [FlowBoard].
  void clear() {
    _groupDatas.clear();
  ...
  /// Returns the [FlowBoardGroupController] with id [groupId].
  FlowBoardGroupController? getGroupController(String groupId) {
  ...
  /// Adds the [FlowBoardGroupItem] to the end of the group
  ///
  /// If the group with id [groupId] is not exist, this method will do nothing.
  void addGroupItem(String groupId, FlowBoardGroupItem item) {
    getGroupController(groupId)?.add(item);
  }

  /// Inserts the [FlowBoardGroupItem] at [index] in the group
  ///
  /// It will do nothing if the group with id [groupId] is not exist
  void insertGroupItem(String groupId, int index, FlowBoardGroupItem item) {
  ...
  /// Replaces or inserts the [FlowBoardGroupItem] to the end of the group.
  ///
  /// If the group with id [groupId] is not exist, this method will do nothing.
  ...
  @override
  FlowBoardGroupController? controller(String groupId) {
    return _groupControllers[groupId];
  }

  @override
  String get identifier => '$FlowBoardController';
  ...
  bool removePhantom(String groupId) {
    ...
    if (isExist) {
      ...
      Log.debug(
          '[$FlowBoardController] Group:[$groupId] remove phantom, current count: ${groupController.items.length}');
    }
    return isExist;
  }
