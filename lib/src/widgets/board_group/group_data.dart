...
abstract class FlowBoardGroupItem extends ReoderFlexItem {
...
/// [FlowBoardGroupController] is used to handle the [FlowBoardGroupData].
///
...
///
class FlowBoardGroupController extends ChangeNotifier with EquatableMixin {
  final FlowBoardGroupData groupData;

  FlowBoardGroupController({
  ...
  /// Returns the readonly List<FlowBoardGroupItem>
  UnmodifiableListView<FlowBoardGroupItem> get items =>
  ...
  ///
  FlowBoardGroupItem? removeAt(int index, {bool notify = true}) {
    ...
    Log.debug('[$FlowBoardGroupController] $groupData remove item at $index');
  ...
  void removeWhere(bool Function(FlowBoardGroupItem) condition) {
  ...
  bool move(int fromIndex, int toIndex) {
    ...
    Log.debug(
        '[$FlowBoardGroupController] $groupData move item from $fromIndex to $toIndex');
  ...
  bool insert(int index, FlowBoardGroupItem item, {bool notify = true}) {
    assert(index >= 0);
    Log.debug('[$FlowBoardGroupController] $groupData insert $item at $index');
  ...
  bool add(FlowBoardGroupItem item, {bool notify = true}) {
    if (_containsItem(item)) {
      return false;
    } else {
      groupData._items.add(item);
  ...
  void replace(int index, FlowBoardGroupItem newItem) {
    if (groupData._items.isEmpty) {
      groupData._items.add(newItem);
      Log.debug('[$FlowBoardGroupController] $groupData add $newItem');
    } else {
      if (index >= groupData._items.length) {
        Log.error(
            '[$FlowBoardGroupController] unexpected items length, index should less than the count of the items. Index: $index, items count: ${items.length}');
        return;
      }
      ...
      Log.debug(
          '[$FlowBoardGroupController] $groupData replace $removedItem with $newItem at $index');
    }

    _notify();
  }

  void replaceOrInsertItem(FlowBoardGroupItem newItem) {
...
/// [FlowBoardGroupData] represents the data of each group of the Board.
class FlowBoardGroupData<CustomData> extends ReoderFlexItem
    with EquatableMixin {
  @override
  final String id;
  FlowBoardGroupHeaderData headerData;
  final List<FlowBoardGroupItem> _items;
  ...
  FlowBoardGroupData({
    ...
    List<FlowBoardGroupItem> items = const [],
  })  : _items = items,
        headerData = FlowBoardGroupHeaderData(
          groupId: id,
          groupName: name,
        );
...
class FlowBoardGroupHeaderData {
  String groupId;
  String groupName;

  FlowBoardGroupHeaderData({required this.groupId, required this.groupName});
}
